 ...
 * @param {'auto' | 'scroll' | 'drag' | 'all'} [props.animationMode]
 * @param {number} [props.radius]
 * @param {number} [props.cardWidth]
 * @param {number} [props.cardHeight]
 * @param {number} [props.verticalSpacing]
 * @param {number} [props.perspective]
 * @param {number} [props.cardsPerTurn]
 * @param {number} [props.rotation]
 * @param {number} [props.cardTilt]
 * @param {number} [props.cardRadius]
 * @param {number} [props.centerScale]
 * @param {number} [props.edgeFade]
 * @param {number} [props.edgeBlur]
...
const InfiniteSpiral = ({
  ...
  return (
    <div
      ref={rootRef}
      className={`relative isolate h-full min-h-80 w-full overflow-hidden bg-white ${className}`}
      style={{
        ...
        {normalizedItems.map((item, index) => {
          ...
          return (
            <Card
              ...
              className="absolute left-1/2 top-1/2 block h-[var(--spiral-height)] w-[var(--spiral-width)] overflow-hidden rounded-[var(--spiral-radius)] text-neutral-900  [backface-visibility:hidden] [transform-style:preserve-3d] [will-change:transform,opacity,filter] motion-reduce:transition-none cursor-pointer hover:border-neutral-900 transition-colors"
              style={{ width: cardWidth, height: cardHeight, borderRadius: cardRadius }}
              href={item.href}
              target={item.target}
              rel={item.target === '_blank' ? 'noreferrer' : undefined}
              role="listitem"
              aria-label={item.label ?? item.name ?? item.alt}
              onClick={() => item.onClick?.(item)}
            >
              {item.icon ? (
                /* Purely centered logo with no text labels */
